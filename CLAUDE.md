...
- **The masthead floats and its height is a token.** `Headers.tsx` is `sticky top-0 z-40`; `--masthead-h` in `global.css` must equal its real rendered height (62px desktop, 74px for the stacked two-row mobile masthead, via a `width < 768px` override). The landing hero + manifesto strip use it to end the first viewport exactly at the strip's bottom edge, and BuildStep's sub-header and the closet sidebar offset their sticky `top` by it. If you change the masthead's padding, type size, or breakpoint, re-measure and update the token. Brand logos on the landing manifesto render in official colors by explicit product decision — the one-accent rule governs our own UI, not third-party marks.
