...
- **The landing stacks its sections on scroll** via `StackPages` (`src/app/components/StackPages.tsx`): sticky pages, a paper veil on the covered page, dependency-free. Pages must carry an opaque background. Feature screenshots live in `frontend/public/screens/` — replace the files, keep the names.
