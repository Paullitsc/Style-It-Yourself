...
- Single Zustand store (`store/styleStore.ts`) manages the full styling flow (5 steps: upload → metadata → colors → build → summary). The build step has its own nested sub-flow (`AddItemPanel`: upload → metadata → colors → validate) for adding additional outfit items. Import directly from `@/store/styleStore`.
- Path alias: `@/*` → `./src/*`
...
- **Style flow state lives only in an in-memory Zustand store — there is no `persist` middleware.** It survives client-side navigation between routes (the store is a JS singleton) but resets on a hard page reload. Visiting `/style` directly with no cropped image but a non-`upload` step triggers an auto-reset (see `src/app/style/page.tsx`).
